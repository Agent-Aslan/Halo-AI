...
ADDITIONAL_REG_WEATHER = os.path.join(DATA_PATH, "merge_final.csv")
REVIEW_MODEL_DATA = os.path.join(DATA_PATH, "review_model_data.csv")

EXT_COLUMNS = ['C1_School closing',
...
class XPrizePredictor(object):
    ...
    def _prepare_dataframe(self, data_url: str) -> pd.DataFrame:
        """Loads the Oxford dataset, cleans it up and prepares the necessary
        columns. Depending on options, also loads the Johns Hopkins dataset and
        merges that in.

        :param data_url: the url containing the original data :return: a
        ...
        # Consciousness context df
        df3 = self._load_consciousness_context_df()
        
        # Merge the Oxford and Consciousness DataFrames
        df13 = df1.merge(df3, on=['GeoID_Date'], how='outer', suffixes=('', '_y'))
        #df13 = df13.loc[:,~df13.columns.str.endswith('_y')]

        # Merge the Oxford/Consciousness(d13) DataFrame with Additional Context
        df = df13.merge(df2, on=['GeoID'], how='left', suffixes=('', '_y'))

        # Set to 0 the GCP missing values
        df['GCP_index'] = df['GCP_index'].replace(np.NaN,0)
        # Drop countries with no population, temperature, humidity, GCP index or planetary-k data
        df.dropna(subset=['Population'], inplace=True)
        df.dropna(subset=['Temperature'], inplace=True)
        df.dropna(subset=['Humidity'], inplace=True)
        df.dropna(subset=['GCP_index'], inplace=True)
        df.dropna(subset=['Planetary_K_index'], inplace=True)

        #  Keep only needed columns
        columns = CONTEXT_COLUMNS + EXT_COLUMNS
        df = df[columns]
        ...
        #Export csv of date for model
        df.to_csv(REVIEW_MODEL_DATA)
    ...
    def _load_original_data(data_url):
        ...
        latest_df["GeoID_Date"] = latest_df["GeoID"] + latest_df["Date"].dt.strftime('%Y%m%d')
        return latest_df
    ...
    @staticmethod
    def _load_consciousness_context_df():
        # File containing the consciousness index for each country and region available
        consciousness_context_df = pd.read_csv(ADDITIONAL_REG_WEATHER,parse_dates=['Date'])
        consciousness_context_df = consciousness_context_df.replace("all",np.NaN)
        consciousness_context_df = consciousness_context_df.replace("nan",np.NaN)
        consciousness_context_df["GeoID"] = np.where(consciousness_context_df["state"].isnull(),
                                                    consciousness_context_df["country"],
                                                    consciousness_context_df["country"] + ' / ' + consciousness_context_df["state"])
        consciousness_context_df.rename(columns={'country': 'CountryName'}, inplace=True)
        consciousness_context_df.rename(columns={'temperature(K)': 'Temperature'}, inplace=True)
        consciousness_context_df.rename(columns={'specific humidity(unitless)': 'Humidity'}, inplace=True)
        consciousness_context_df.rename(columns={'GCP index(1 hour chisquare -> Z)': 'GCP_index'}, inplace=True)
        consciousness_context_df.rename(columns={'planetary K-index(space)': 'Planetary_K_index'}, inplace=True)
        del consciousness_context_df['state']
        consciousness_context_df["GeoID_Date"] = consciousness_context_df["GeoID"] + consciousness_context_df["Date"].dt.strftime('%Y%m%d')
        return consciousness_context_df

    @staticmethod
    def _create_country_samples(df: pd.DataFrame, geos: list) -> dict:
        """For each country, creates numpy arrays for Keras.

        :param df: a Pandas DataFrame with historical data for countries
        (the "Oxford" dataset) :param geos: a list of geo names :return:
        a dictionary of train and test sets, for each specified country
        """
    ...
    def _most_affected_geos(df, nb_geos, min_historical_days):
        """Returns the list of most affected countries, in terms of confirmed
        deaths.

        :param df: the data frame containing the historical data :param
        nb_geos: the number of geos to return :param
        min_historical_days: the minimum days of historical data the
        countries must have :return: a list of country names of size
        nb_countries if there were enough, and otherwise a list of all
        the country names that have at least min_look_back_days data
        points.
        """
